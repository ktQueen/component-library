/**
 * 表单行配置组件
 * @module form_complex_paraConfig
 */
define([
     ...
     * @param {string} opts.type='paraConfig' - 必填，要创建的表单字段的类型
     ...
     *
     * @param {array} [opts.childrenList=[] - 行内容，只用配置一行的数据，如果要设置值请用setValue方法
     *
     * @requires 'jquery'
     * @requires 'require'
     */
    var FormParaConfig=function(){};
    FormParaConfig.prototype = {
        /*初始化*/
        init: function(opts) {
            var _this = this;
            opts.childrenTbody=$.extend(true,[],opts.childrenList);
        ...
        ownClass:'form-complex-paraConfig',//根据不同的类型给每个字段添加私有的class
    };
    Manager.FormComplexParaConfig= Base.extend(FormParaConfig);
    return Manager.FormComplexParaConfig;
});