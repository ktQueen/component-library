...
! function(e) {
    "function" == typeof define && define.amd ?
        define(["jquery", "moment"], function(j, m) {
    ...
    e.dateRangePickerLanguages = {
            "default": {
                ...
                "default-selectMonth": "Please select a month",//只选择月份
            ...
            cn: {
                ...
                "default-selectMonth": "请选择一个月份",//只选择月份
        ...
        e.fn.dateRangePicker = function(a) {
            ...
            function r() {
                ...
                var h = "";
                //当有月份的时候默认显示月份的提示语
                h = te( a.selectMonth ? "default-selectMonth" : a.singleDate ? "default-single" : a.minDays && a.maxDays ? "default-range" : a.minDays ? "default-more" : a.maxDays ? "default-less" :"default-default"),
                    ...
                    re.find(".apply-btn").on("click", re, function(evt) {//确定按钮点击事件
                        A();
                        if (a.selectMonth) {//当点击确定的时候，如果是只选择月份的时候执行
                            var sels = evt.data.find("select");
                            a.start = sels[1].value + "-" + (parseInt(sels[0].value) + 1);
                            t = V(new Date(a.start)).substring(0, 7);
                            l.val(t);
                            re.find(".default-top").hide();
                            re.find('.normal-top').show().find('.start-day').html(t);
                        } else {
                    ...
                    re.find(".empty-btn").click(function(evt) {//清空按钮点击事件
                        evt.stopPropagation();
                        f();
                        A();
                        if (a.selectMonth) {//当点击确定的时候，如果是只选择月份的时候执行
                            re.find(".default-top").show();
                            re.find('.normal-top').hide();
                        }
            ...
            function s() {
                if (!a.inline) {
                    ...
                    if ("relative" == e(a.container).css("position")) {
                        ...
                        //top值设置
                        var screenHeight = e(window).height() + document.body.scrollTop;//屏幕的高度+被卷去的高度
                        //屏幕的高度-按钮元素到页面顶部的距离-按钮的高度+2>菜单的高度
                        if (screenHeight - t.top - e(de).outerHeight() >= e(re).outerHeight()) {//如果下面区域放得下菜单
                            top = t.top + e(de).outerHeight();
                        } else {//如果下面区域放不下就放在上面
                            top = t.top - e(re).outerHeight();
                        }
                        if(top<0){//如果上面放不下，下面放不下就居中显示
                            top=(e(window).height()- e(re).outerHeight())/2+document.body.scrollTop;
                        }
                        re.css({
                            top: top,
                            left: left
            ...
            function u(sM) {
                ...
                sM ? re.find(".month-wrapper").height(40) : '';//当是只选择月份的时候下拉框的高度设置
            }
            ...
            function C(t, n) {
                re.find(".start-day").html("..."),
                    ...
                    a.start && re.find(".start-day").html(a.selectMonth?V(new Date(a.start)).substring(0, 7):V(new Date(parseInt(a.start)))),
            ...
            function Y(e, n) {
                ...
                var mStr = "<select>"
                for (var i = 0; i < 12; i++) {
                    mStr += "<option value=\'" + i + "\' " + (e.getMonth() == i ? "selected" : "") + ">" + te("month-name")[i] + "</option>"
                }
                mStr += "</select>";
                var $monthSel = jQuery(mStr);
                $monthSel.change(function(evt) {
                ...
                var yStr = "<select>";
                if (a.startEndYear) {
                    var startY = a.startEndYear.split('-')[0];
                    var endY = a.startEndYear.split('-')[1];
                }
                for (var i = startY; i <= endY; i++) {
                    yStr += "<option value=\'" + i + "\' " + (e.getFullYear() == i ? "selected" : "") + ">" + i + "</option>"
                }

                yStr += "</select>"
                var $yearSel = jQuery(yStr);
                $yearSel.change(function(evt) {
                    var y = jQuery(this).val();
                    e.setFullYear(y);
                    Y(e, n);
                });
                re.find("." + n + " .month-name").empty().append($monthSel).append($yearSel),
            ...
            function A() {
                if (!a.alwaysOpen) {
                    ...
                    a.customCloseAnimation ? a.customCloseAnimation.call(re.get(0), t) : e(re).slideUp(0, t),
            ...
            function q() {
                ...
                //当是只选择月份的情况就不生成下面的日期表格
                if (t += '<div class="month-wrapper"><table class="month1" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="' + n + '" class="month-name"></th><th style="width:27px;">' + (a.singleDate || !a.stickyMonths ? '<span class="next">&gt;</span>' : "") + '</th></tr>' + (!a.selectMonth ? '<tr class="week-name">' + E() + "</thead><tbody></tbody></table>" : ''), G() && (t += '<div class="gap">' + J() + '</div><table class="month2" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;">' + (a.stickyMonths ? "" : '<span class="prev">&lt;</span>') + '</th><th colspan="' + n + '" class="month-name"></th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">' + E() + "</thead><tbody></tbody></table>"), t += '<div style="clear:both;height:0;font-size:0;"></div><div class="time"><div class="time1"></div>', a.singleDate || (t += '<div class="time2"></div>'), t += '</div><div style="clear:both;height:0;font-size:0;"></div></div>', t += '<div class="footer">', a.showShortcuts) {
            ...
            a || (a = {}),
                a = e.extend(!0, {
                        ...
                        applyBtnClass: "",
                        singleMonth: "auto",
                        selectMonth: false,//默认不只是只选择月份的配置
